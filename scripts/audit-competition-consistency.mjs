 ...
 * 不需連外網就能抓出來。
 *
 * 效力實測（以 #69 修復前的 133 筆為基準，對照該 PR 實際修掉的 37 筆問題條目）：
 *
 *   命中 27 筆，其中真陽性 10 筆
 *   recall 27%（漏掉 27 筆）、precision 37%
 *
 * 也就是說它**漏掉約四分之三的真實缺陷**，而且抓不到本專案最主要的問題類型：
 * 失效連結、連到機構首頁而非競賽頁、整筆捏造的賽事、標題混淆兩個不相干競賽。
 * 那些只能靠連結健檢（check-competitions.mjs）與人工逐頁查證官網。
 *
 * 因此本工具的定位是**補充性的交叉檢查**，不是稽核的主要手段，更不可當成
 * CI 的通過條件。誤判來源主要為否定句、日期字串、縣市初賽的「選拔」等。
 *
